...
export default function ProductForm({ initial }: ProductFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-6 max-w-2xl">
      ...
      {/* Detalles del producto (features) */}
      <div className="flex flex-col gap-2">
        <div className="flex items-baseline justify-between">
          <label className="text-xs uppercase tracking-widest text-gray-500 font-medium">Detalles del producto</label>
          <span className="text-[10px] text-gray-400">Aparece en el acordeón de la página del producto</span>
        </div>
        ...
        {(form.features ?? []).length > 0 && (
          <ul className="flex flex-col gap-1.5">
            {(form.features ?? []).map((f, i) => (
              <li key={i} className="flex items-center gap-2 bg-gray-50 border border-gray-100 rounded-lg px-3 py-2">
          ...
          <input
            ...
            onKeyDown={e => {
              if (e.key === 'Enter') {
                ...
                if (v) { setField('features', [...(form.features ?? []), v]); setFeatureInput(''); }
              }
            }}
            placeholder="Ej: Tela premium 78% Nylon, 22% Spandex"
            className="border border-gray-200 focus:border-gray-400 rounded-lg px-3 py-2 text-sm focus:outline-none transition-colors flex-1"
          />
          <button
            type="button"
            onClick={() => {
            ...
            className="text-xs font-semibold text-white bg-black hover:bg-gray-800 px-4 py-2 rounded-lg transition-colors shrink-0"
          >
            + Agregar
          </button>
        </div>
        <p className="text-[10px] text-gray-400">Presiona Enter o el botón para agregar cada característica por separado</p>
      </div>
